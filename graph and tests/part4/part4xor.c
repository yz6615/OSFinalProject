...
int main(int argc, char *argv[]) {
    FILE *fWrite = fopen("part4Data.txt","w");
    //FILE *fWriteWithCache = fopen("dataWithCache.txt","w");
    char * fileName; //file name
    int blockSize = 64;
    int blockCount = 1592012;
    int ratio;
    char command[80];

    strcpy(command, "sudo sh -c \"/usr/bin/echo 3 > /proc/sys/vm/drop_caches\"");
    if (argc != 3) {
    ...
    if (compareArrays(argv[2],"-r")) {
	    ...
	    ratio = 8192 / blockSize;
	    system(command);
            file_read(blockSize, blockCount*ratio, fileName, fWrite);
	    file_read(blockSize, blockCount*ratio, fileName, fWrite);
            blockSize *= 2;
        }
    }
